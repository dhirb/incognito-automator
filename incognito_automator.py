...
class IncognitoAutomator():
	def __init__(self, browser_choice, card_to_search):
		...
		try:
			...
			print('{card} does not exist in the config file'.format(card=card_to_search))
			exit(1)
		except IOError:
	...
	def get_best_offer(self):
		...
		if re.search(self.search_term, self.browser.page_source):
			# DO NOT close browser here, you want to apply for the best offer don't you?
...
parser = argparse.ArgumentParser(description='Script to get best signup offer on credit cards')
parser.add_argument('-b', '--browser', action='store', choices=['chrome', 'firefox'], default='chrome',
...
for attempt in range(args.attempt):
	print('Attempt {attempt} of {limit}'.format(attempt=attempt + 1, limit=args.attempt))

	automator = IncognitoAutomator(args.browser, args.card)
	automator.get_best_offer()

print('Failed to find best offer after {attempt} attempts.'.format(attempt=args.attempt))
